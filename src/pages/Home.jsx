...
export default function Home() {
  ...
  useEffect(() => {
    base44.auth.me().then(async (user) => {
      ...
      if (user.auto_play_last_session) {
        setTimeout(async () => {
          try {
  ...
  useEffect(() => {
    if (activeSounds.length === 0 || !currentUser) return;
  ...
  const handleToggleSound = useCallback((sound) => {
    setActiveSounds((prev) => {
      ...
      if (exists) {
        ...
        if (remaining.length === 0) audioEngine.stopSound("_base_background");
        return remaining;
      } else {
  ...
  }, [audioEngine, currentUser, analytics, activityTracker]);
  ...
  const handleReplayHistory = useCallback((historyItem) => {
    audioEngine.stopAll();
    setActiveSounds([]);
    setTimeout(() => {
...
Think about what sounds naturally fit the mood and setting described. Vary volumes thoughtfully.`,
  ...
  const handleSelectFavoriteSound = useCallback((soundId) => {
    ...
    if (sound) handleToggleSound(sound);
  }, [handleToggleSound]);

  const handleSelectFavoritePreset = useCallback(async (presetId) => {
  ...
  return (
    <div data-tab-content className="min-h-screen bg-slate-950 text-white relative overflow-hidden overflow-y-auto" {...pullHandlers}>
      ...
      <div className="fixed inset-0 pointer-events-none overflow-hidden">
        <motion.div className="absolute top-[-20%] left-[-10%] w-[60vw] h-[60vw] rounded-full bg-green-950/30 blur-[120px]" animate={activeSounds.length > 0 ? { scale: [1, 1.05, 1], opacity: [0.3, 0.45, 0.3] } : {}} transition={{ duration: 8, repeat: Infinity, ease: "easeInOut" }} />
        <motion.div className="absolute bottom-[-20%] right-[-10%] w-[50vw] h-[50vw] rounded-full bg-emerald-950/25 blur-[120px]" animate={activeSounds.length > 0 ? { scale: [1, 1.08, 1], opacity: [0.2, 0.35, 0.2] } : {}} transition={{ duration: 10, repeat: Infinity, ease: "easeInOut", delay: 1 }} />
        <motion.div className="absolute top-[40%] left-[10%] w-[30vw] h-[30vw] rounded-full bg-teal-950/20 blur-[100px]" animate={activeSounds.length > 0 ? { scale: [1, 1.1, 1], opacity: [0.15, 0.3, 0.15] } : {}} transition={{ duration: 7, repeat: Infinity, ease: "easeInOut", delay: 2 }} />
      </div>
      <div className="relative z-10 px-4 sm:px-8 pt-6 pb-36">
        <div className="max-w-3xl mx-auto">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.15 }} className="text-center mb-6">
            <div className="flex items-center justify-center gap-3 mb-2">
              <motion.span className="text-3xl sm:text-4xl" animate={activeSounds.length > 0 ? { rotate: [0, 5, -5, 0], scale: [1, 1.1, 1] } : {}} transition={{ duration: 4, repeat: Infinity }}>🌿</motion.span>
            ...
            <p className="text-white/30 text-xs mt-2">🎵 Click a sound, then tap <span className="text-emerald-400/70 font-medium">Forest Base</span> below to hear it</p>
          </motion.div>
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.3 }} data-tour="sound-mixer">
            <GardenMixer activeSounds={activeSounds} onToggleSound={handleToggleSound} onVolumeChange={handleVolumeChange} favorites={favorites} onToggleFavorite={handleToggleSoundFavorite} />
          </motion.div>
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.8, delay: 0.5 }} className="flex flex-wrap justify-center items-center gap-2 mt-7">
            {currentUser && (
              <>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.8, delay: 0.65 }} className="mt-6 p-4 rounded-2xl bg-white/[0.03] border border-white/[0.06] backdrop-blur-xl">
      ...
      <SavePresetDialog isOpen={saveDialogOpen} onClose={() => setSaveDialogOpen(false)} activeSounds={activeSounds} />
